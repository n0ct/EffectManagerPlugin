...
public class ItemProjectile extends EntityItem implements IProjectile, CustomProjectile {
    ...
    public ItemProjectile(String name, Location origin, LivingEntity player, ItemStack item, float power) {
    	super(((CraftLivingEntity) player).getHandle().world,player.getLocation().getX(), player.getLocation().getY(), player.getLocation().getZ());
        lastTick = MinecraftServer.currentTick;
        this.name = name;
        this.pickupDelay = Integer.MAX_VALUE;
        setItemStack(CraftItemStack.asNMSCopy(item));
        this.shooter = ((CraftLivingEntity) player).getHandle();
        this.a(0.25F, 0.25F);
