...
public class TNTProjectile extends EntityTNTPrimed implements CustomProjectile, IProjectile {
    ...
    public TNTProjectile(String name, LivingEntity shooter, float power) {
        super(((CraftLivingEntity) shooter).getHandle().world, shooter.getLocation().getX(), shooter.getLocation().getY(), shooter.getLocation().getZ(), ((CraftLivingEntity) shooter).getHandle());
        this.name = name;
        this.fuseTicks = 20;
        lastTick = MinecraftServer.currentTick;
        this.a(0.25F, 0.25F);
    ...
    public TNTProjectile(String name, LivingEntity player, Location origin, float power) {
        super(((CraftWorld) origin.getWorld()).getHandle(), origin.getX(), origin.getY(), origin.getZ(), ((CraftLivingEntity) player).getHandle());
        this.name = name;
        this.fuseTicks = 20;
        lastTick = MinecraftServer.currentTick;
        this.a(0.25F, 0.25F);
    	setPosition(origin.getBlockX(), origin.getBlockY(), origin.getBlockZ());
        shoot(origin.getBlockX(), origin.getBlockY(), origin.getBlockZ(), power * 1.5F, 1.0F);
	}
